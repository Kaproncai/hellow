; Windows 512 bytes intro 3D by TomCat/Abaddon
...
;midiOutOpen(&handle, 0, 0, 0, CALLBACK_NULL);
 PUSH ESI      ; lphmo
 LODSD      
 CALL DWORD [ESI-start-4+midiOutOpen]

;ShowCursor(FALSE);
 CALL DWORD [ESI-start-4+ShowCursor]

 LEA EAX,[ESI-start-4+edit]
...
;CreateWindowEx(0,"edit",0,WS_POPUP|WS_MAXIMIZE|WS_VISIBLE,0,0,0,0,0,0,0,0);
 CALL DWORD [ESI-start-4+CreateWindowExA]

 MOV EDX,ESP
 PUSH EAX      ; hwnd
 PUSH EDX      ; RECT
 PUSH EAX      ; hwnd
 CALL DWORD [ESI-start-4+GetWindowRect]

;GetDC(hwnd);
 CALL DWORD [ESI-start-4+GetDC]
 PUSH EAX      ; hdc

music:
 MOV EDX,007F1090H
@@:
 BT [ESI-start-4+score],EBX
 RCL DH,1
 INC BL
 JNC @B
 ADD DH,[ESI-start-4+adder]
 TEST BL,BL
 JNZ @F
 XOR WORD [ESI-start-4+last],7420H
@@:

;midiOutShortMsg(handle, 0x007f4090);
 PUSH EDX      ; send note on channel 0
 PUSH DWORD [ESI-4]
 CALL DWORD [ESI-start-4+midiOutShortMsg]

 PUSH 29       ; tempo of the music
 POP EBP

main:
 CALL DWORD [ESI-start-4+GetTickCount]
 CMP EAX,EDI
 JE main
 XCHG EDI,EAX  ; time counter
...
;StretchDIBits(hdc,rc.left,rc.top,rc.right,rc.bottom,0,0,ResX,ResY,pixels,bmpnfo,0,SRCCOPY);
 CALL DWORD [ESI-start-4+StretchDIBits]
 SUB ESP,13*4  ; repair the stack frame (preserves StretchDIBits arguments)

 PUSH 1BH      ; VK_ESCAPE
 CALL DWORD [ESI-start-4+GetAsyncKeyState]
 TEST EAX,EAX
 JZ main

quit:
 JMP DWORD [ESI-start-4+ExitProcess]
